...
class Crawler():
	...
	def __init__(self, parserobots=False, output=None, report=False ,domain="",
				 exclude=[], skipext=[], drop=[], debug=True, verbose=False, images=False):
	...
	def __crawling(self):
		...
		if crawling in self.crawled:
			logging.info("Already crawled "+crawling)
			return
		...
		if not url.path.endswith(self.not_parseable_ressources):
			...
			logging.info("Ignore {0} content might be not parseable.".format(crawling))
			response = None
		...
		if response is not None:
			try:
				...
				logging.info ("{1} ===> {0}".format(e, crawling))
				return None
		else:
		...
		self.analyzeUrlAndSave(image_list, lastmod, url)
		...
		for link in links:
			...
			if link in self.excluded:
				continue
			if (domain_link != self.target_domain) and (domain_link != "http://download.music.com.bd"):
			...
			self.tocrawl.add(self.replaceSpaces(link))

		return None


	def replaceSpaces(self, text):
		return text.replace(" ", "%20")

	def analyzeUrlAndSave(self, image_list, lastmod, url):
		if "www.music.com.bd" in url.geturl():
			if ".mp3.html" in url.geturl():
				# print("<url><loc>" + self.htmlspecialchars(url.geturl().replace("www.music.com.bd/download", "download.music.com.bd")) + "</loc>" + lastmod + image_list + "</url>",
				# 	  file=self.output_file)
				urlToWrite =  self.htmlspecialchars(url.geturl().replace("www.music.com.bd/download", "download.music.com.bd").replace(".mp3.html", ".mp3"))

				if urlToWrite not in self.savedToFile:
					self.savedToFile.append(urlToWrite)
					self.pingAndWriteToFile(urlToWrite)
				else :
					logging.info("Not writing Url already exist "+url.geturl())
			else:
				logging.info("Not writing " + url.geturl())
		elif ("download.music.com.bd" in url.geturl()):
			urlToWrite = self.htmlspecialchars(url.geturl())
			if url.geturl().endswith(".mp3") and (urlToWrite not in self.savedToFile):
				self.savedToFile.append(urlToWrite)
				self.pingAndWriteToFile(urlToWrite)
			else:
				logging.info("Not writing Url already exist " + url.geturl())
		else :
			logging.info("Not writiing domain miss matched "+url.geturl())

		if self.output_file:
			self.output_file.flush()

	def pingAndWriteToFile(self, urlToWrite):
		if self.ping(urlToWrite):
			logging.info("Writing to file "+urlToWrite)
			print("" + urlToWrite + "", file=self.output_file)

	def ping(self, urlToPing):
		c = http.client.HTTPConnection('www.example.com')
		c.request("HEAD", '')
		if c.getresponse().status == 200:
			return True
		else:
			print("web site does't exists "+urlToPing)
			return False
